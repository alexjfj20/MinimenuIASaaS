...
async function main() {
    ...
    const testOrder = {
        ...
        customer_name: 'Debug Script User',
    ...
    if (error) {
        ...
        if (error.code === '42501') {
            console.error("CAUSA PROBABLE: Política RLS (Row Level Security) impide la inserción pública.");
        }
    } else {
        console.log("INSERCIÓN EXITOSA. ID del Pedido:", data.id);
        console.log("Esto significa que la base de datos permite crear pedidos correctamente.");
    }
}
